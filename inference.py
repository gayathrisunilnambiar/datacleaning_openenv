...
def run_task(
    ...
    for _ in range(MAX_LOGICAL_STEPS):
        ...
        rewards.append(float(step_payload.get("reward", 0.0)))

        if step_payload.get("done"):
            final_score = float(step_payload.get("info", {}).get("grader_score", 0.0))
            emit(
                "[END]",
                {
    ...
    final_score = float(observation.get("episode_reward_so_far", 0.0))  # fallback
    emit(
        "[END]",
        {
            "success": False,
            "steps": observation.get("step_number", 0),
            "score": None,
            "rewards": rewards,
        },
    )
