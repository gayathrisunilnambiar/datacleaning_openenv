...
The agent receives a structured state at each step:

| Field | Type | Description |
| :--- | :--- | :--- |
| `task_id` | `string` | Unique identifier for the active task. |
| `step_number` | `int` | Logical step counter (0 to `max_steps`). |
| `current_df` | `list[dict]` | The current table data in JSON records format. |
| `dirty_columns` | `list[str]` | List of columns that currently fail the similarity check. |
| `columns_meta` | `dict` | Current pandas dtypes for each column. |
| `episode_reward` | `float` | Cumulative reward collected in the episode. |
| `done` | `bool` | Whether the episode is finished. |
| `task_description` | `string` | High-level instructions for the agent. |

The benchmark combines:

- dense per-step reward shaping
- a no-op penalty (`-0.05`) for redundant actions
